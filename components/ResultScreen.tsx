...
export function ResultScreen({
  ...
  return (
    <div className="flex flex-1 flex-col gap-5 animate-fade-in">
      <p className="text-center text-sm text-[var(--quiz-muted)]">
        你的麵包人格是…
      </p>
      ...
      >
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <h2 className="text-2xl font-bold text-[var(--quiz-text)]">
            {result.name}
          </h2>
          <div className="flex flex-wrap justify-center gap-2">
            {result.keywords.map((kw) => (
              <span
                key={kw.text}
                className="rounded-full bg-white px-3 py-1 text-sm text-[var(--quiz-text)] shadow-sm"
              >
                {kw.emoji} {kw.text}
              </span>
            ))}
        ...
        <div className="mt-5 space-y-4 text-sm leading-relaxed text-[var(--quiz-text)]">
          <section>
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-[var(--quiz-muted)]">
              人格敘述
            </h3>
          ...
          <section className="rounded-xl bg-white/80 p-4">
            <h3 className="mb-1 text-xs font-semibold text-[var(--quiz-muted)]">
              適合飲料
            </h3>
      ...
      {shareImageUrl && (
        <div className="space-y-2">
          <p className="text-center text-xs text-[var(--quiz-muted)]">
            長按下方圖片可儲存到相簿
          </p>
      ...
      <div className="flex flex-col gap-2.5 sm:flex-row">
        <button
          ...
          onClick={onSave}
          disabled={isSaving}
          className="quiz-btn-primary flex-1 rounded-full px-6 py-3 text-sm font-semibold disabled:opacity-60 sm:text-base"
        >
          {isSaving ? "產生圖片中…" : "儲存結果"}
        </button>
        <button
          type="button"
          onClick={onRetake}
          className="flex-1 rounded-full border border-[var(--quiz-border)] bg-white px-6 py-3 text-sm font-semibold text-[var(--quiz-text)] transition-colors hover:bg-[var(--quiz-option-hover)] sm:text-base"
        >
          重新測一次
        </button>
      </div>
